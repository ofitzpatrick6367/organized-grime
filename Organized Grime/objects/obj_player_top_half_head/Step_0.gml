...
if (instance_exists(obj_dialogue_parent)) {
    can_move = false;
    if (sprite_index == spr_player_walk_right_top_head) sprite_index = spr_player_idle_right_top_head;  
    else if (sprite_index == spr_player_walk_left_top_head) sprite_index = spr_player_idle_left_top_head;
    else if (sprite_index == spr_player_walk_down_top_head) sprite_index = spr_player_idle_down_top_head;
    else if (sprite_index == spr_player_walk_up_top_head) sprite_index = spr_player_idle_up_top_head;
}
...
move_and_collide(_hor * move_speed, _ver * move_speed, [tilemap, mess, npc, safe, fridge, stair, dogbed, level_one_npc, level_one_npc_2, desk, swivel_chair, bush], undefined, undefined, undefined, move_speed, move_speed);
    ...
    if (_hor != 0 or _ver != 0)
    {
        if (_ver > 0) sprite_index = spr_player_walk_down_top_head; 
        else if (_ver < 0) sprite_index = spr_player_walk_up_top_head;
        else if (_hor > 0) sprite_index = spr_player_walk_right_top_head;
        else if (_hor < 0) sprite_index = spr_player_walk_left_top_head;
